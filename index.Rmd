...
### Percentage of total population vaccinated {.no-mobile}
```{r echo=FALSE}
vac_packed_bubble
```

### Percentage of total population vaccinated {.mobile}
```{r echo=FALSE}
vac_packed_bubble %>%
...
### <span style="font-size:1.3em;"> **New cases per million | Split Bubble plot ** </span>
```{r echo=FALSE, fig.height=6, fig.width=7, warning=FALSE}
...
***
All countries are grouped (colour-coded) by continents, except Malaysia where it is represented by a unique colour. Each bubble represents a country, with its radius corresponding to the number of new cases per million population. Hover your cursor over or tap the bubbles to explore the number of daily cases per million population in different countries. If you are using a computer, you can highlight a continent by hovering your cursor over the legend text (at the bottom of the plot) associated with that continent. To deselect a continent, click or tap on the legend text. (you can therefore zoom in on a continent of interest by deselecting all other continents).

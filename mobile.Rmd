...
### Vaccinations - Daily Total & Cumulative Total
```{r include=FALSE}
...
### Share of minimum target population (23.2m) already vaccinated
```{r echo=FALSE}
...
```{r echo=FALSE, fig.height=3, fig.width=6, warning=FALSE, fig.align='left'}
...
```{r include=FALSE}
source('cases/daily_new_cases.R')
col<-brewer.pal(9 ,'Oranges')[c(3,7)]
```
>
7-day total <span style="font-size:0.8em; color:grey;"> *(week ending `r lastlastdate`)*</span>  
<span style="font-size:2em; color:orange;"> **`r format(sum(lastlast7cases), scientific=F, big.mark=',')`** </span>    
7-day total <span style="font-size:0.8em; color:grey;"> *(week ending `r format(max(x), '%d %b')`)*</span>  
<span style="font-size:1.5em; color:orange;"> **`r format(sum(last7cases), scientific=F, big.mark=',')`** </span> &nbsp;
<span style="font-size:1.2em; color:`r arrowcol`;"> **`r ifelse(weekchange>=0, print('⬆'), print('⬇'))`  `r label`**</span>  
...
names(data)[c(2,3)] <- c("Tests", "Positive")
data$date <- as.Date(data$date)
# convert data frame into xts format
data <- xts(data[,2:3], order.by = data$date)         
...
>
*`r format(max(test$x, na.rm=T), '%d %B %Y')`*   
<span style="font-size:2em; color:`r col[1]`;"> **`r latest_tests`** </span>  
...
>
<span style="font-size:2em; color:`r col`;"> **`r latest_pr`%** </span>  
<span style="color:grey;"> *Daily positivity rate.* </span>   
<span style="font-size:1.5em; color:`r col`;"> **`r pos7avg`%** </span>  
<span style="color:grey;"> *7-day moving average (week ending `r format(x[length(pos)-6], '%d %b')`).* </span>  
&nbsp;    
...
# Plot!
dygraph.plot(data, col, dates=mys$date, legend.width=100, vr=c(0, max(data[,1], na.rm=T)+1e5) ) %>%
  dySeries("V1", label = "Total") %>%
  dyLegend('follow', labelsSeparateLines = T, width=100)
```
...
<span style="font-size:2em; color:`r col`;"> **`r format(max(y, na.rm=T), scientific=F, big.mark=',')`** </span>  
<span style="color:grey;"> *Cumulative number of cases.* </span>  
<span style="font-size:2em; color:`r col`;"> **`r format(round(max(mys$total_cases_per_million), 0), scientific=F, big.mark=',')`** </span>  
...
### <span style="font-size:1.3em;"> **Percentage Vaccinated |** </span> <br> <span style="font-size:0.9em;"> What share of the population has been vaccinated *based on [2020 population estimates](https://www.dosm.gov.my/v1/index.php?r=column/cthemeByCat&cat=155&bul_id=OVByWjg5YkQ3MWFZRTN5bDJiaEVhZz09&menu_id=L0pheU43NWJwRWVSZklWdzQ4TlhUUT09) of 32.7 million?* </span>
```{r echo=FALSE, fig.height=3, fig.width=6, warning=FALSE, fig.align='left'}
# configure data frame for dygraph
...
>
*`r format(max(x, na.rm=T), '%d %B %Y')`*  
<span style="font-size:1.5em; color:`r col[1]`;"> **`r round(max(y1), 1)`%** </span> <span style="color:grey;"> *have received at least 1 dose of a vaccine.* </span>   
<span style="font-size:1.2em; color:`r col[1]`;"> **+ `r inc1`%** </span> <span style="color:grey;"> *from `r format(d$date[length(d$date)-1], '%d %b')`* </span>  
<span style="font-size:1.5em; color:`r col[2]`;"> **`r round(max(y2), 1)`%** </span> <span style="color:grey;"> *have received at least 2 doses of a vaccine.* </span>   
<span style="font-size:1.2em; color:`r col[2]`;"> **+ `r inc2`%** </span> <span style="color:grey;"> *from `r format(d$date[length(d$date)-1], '%d %b')`* </span>
...
# Plot!
dygraph.plot(data, col, fill=FALSE, dates=d$date, legend.width=120, display='always',
             vr=c(0, max(data[,1], na.rm=T)+2e4 ))
```
...
names(data)[c(2,3)] <- c("1st dose (%)", "2nd dose (%)")
future <- data.frame(date=seq(max(data$date)+1, as.Date('2022-02-28'), 1), '1st' =0, '2nd' =0 )
...
# col [1] for 2 doses [2] for 1 dose
col<- brewer.pal(9 ,'Greens')[c(5,9)]
phase <- brewer.pal(9 ,'Blues')[c(4,6,8)]
# Plot!
dygraph.plot(data, col, 
  ...
  dyLimit(72.2, color = phase[3], label='Phase 3', labelLoc='right')                   %>%
  dyLimit(30.3, color = phase[2], label='Phase 2', labelLoc='right')                   %>%
  dyLimit(1.5, color = phase[1], label='Phase 1', labelLoc='right') 
```
...
>
<span style="font-size:2em; color:`r col[3]`;"> **`r dailydose1`** </span>
<span style="color:grey;"> *first doses* </span>  
<span style="font-size:2em; color:`r col[2]`;"> **`r dailydose2`** </span>
...
>
<span style="font-size:2em; color:`r col[2]`;"> **`r death7avg`** </span> <span style="font-size:1em; color:grey;"> **deaths / day** </span>  
...
# Plot!
dygraph.plot(data, col[2], legend.width=140, display='onmouseover', 
             vr=c(0, max(data[,1], na.rm=T)+0.5) )                      %>%
  dySeries("V1", label = "Case fatality ratio (%)") %>%
    dyLegend('follow', labelsSeparateLines = T, width=140)
```

>
<span style="font-size:2em; color:`r col[2]`;"> **`r cfr`%** </span>  
<span style="color:grey;"> *Case Fatality Ratio (CFR) or more commonly known as Case Fatality Rate.* </span>    
&nbsp;  
<span style="color: Grey;"> *[CFR](https://www.who.int/news-room/commentaries/detail/estimating-mortality-from-covid-19) is the total number of deaths divided by the total number of confirmed cases (up to and including *`r format(max(x, na.rm=T), '%d %B %Y')`*). Despite being a commonly used measure of mortality from a disease, it has a number of important limitations and underlying assumptions which call for careful interpretation, especially during a fast-moving pandemic. Read [here](https://ourworldindata.org/covid-mortality-risk) for instance.* </span>
...
# Plot!
dygraph.plot(data, col, legend.width=120, display='onmouseover',
             vr=c(0, max(data[,1], na.rm=T)+2e3) )                    %>%
  dySeries("V1", label = "Cumulative deaths") %>%
    dyLegend('follow', labelsSeparateLines = T, width=120)
```
...
>
*Latest data provided on `r format(max(x, na.rm=T), '%d %B %Y')`*  
<span style="color:`r col[2]`; font-size:2em;">**`r cum_deaths`**</span> 
